...
| RAG search | Shipped | `RAGStore`, `RAGEngine`, `RAGSearchTool` | Empty results, embedding failures, fetch failures, lexical degradation, and stale embedding metadata are surfaced. Unversioned/model- or dimension-mismatched vectors require explicit reindexing. |

...
| Dataset and adapter repositories could become public through omitted opt-in privacy flags or retain stale visibility when they already existed. | Space, dataset, and adapter/model repositories are private by default. The builder explicitly updates and reads back each repository's requested visibility before any upload. Visibility changes require the repository-specific `--public-*` override, and a public Space still requires the independent admin secret. |
...
Existing repositories are updated and their actual visibility is read back before the first upload;
a settings error, unknown state, or mismatch fails closed.

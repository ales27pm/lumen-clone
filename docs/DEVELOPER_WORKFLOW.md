...
7. **Optional training/HF profile** remains opt-in. `--with-training-plan` prints the `ubuntu-preflight`, `train-adapters`, `hf-resolve`, and `hf-upload-dry-run` jobs. The Ubuntu preflight and training jobs both route through the pinned one-click container launcher; conversion and frozen evaluation are phases of that run. `--run-training` is required to execute them.

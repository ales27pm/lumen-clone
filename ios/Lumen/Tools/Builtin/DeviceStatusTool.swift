...
struct DeviceStatusTool: LocalTool {
    ...
    func execute(invocation: ToolInvocation, context: ToolExecutionContext) async -> ToolResult {
        ...
        let text = "OS: \(snap.osVersion), Metal: \(snap.metalAvailable), CoreML embeddings: \(snap.coreMLStatus), FoundationModels: \(snap.foundationModelsStatus), LPM: \(snap.lowPowerModeEnabled), Thermal: \(snap.thermalState.rawValue)"

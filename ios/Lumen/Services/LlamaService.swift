...
final actor AppLlamaService {
    ...
    private func recordModelTrace(
        ...
        let parsed = AgentTurnParser.parse(output)
        let selectedToolID = parsed.action.map { ToolRouteGuard.canonicalToolID($0.tool) }
        AgentBehaviorTraceEmitter.recordModelTurn(
            ...
            toolArguments: parsed.action?.args.stringCoerced ?? [:],

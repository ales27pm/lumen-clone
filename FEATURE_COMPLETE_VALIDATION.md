...
Separately, a no-upload Release archive/export completed from exact source `5df950aeb58088d2cf5a371a0b373a8ca9377d5e` with build number `20260810035524`. `xcodebuild` reported `** ARCHIVE SUCCEEDED **` for `build/Lumen-20260809-235633.xcarchive` and `** EXPORT SUCCEEDED **` for `build/export-Lumen-20260809-235633/Lumen.ipa`; the exported IPA had SHA-256 `760c1a4f8f7444211229bf73d7804fb1e5dd90ff36082c7f32615b35b27c9a83`. The wrapper verified archive and IPA Info.plist/provenance, the app privacy manifest, bundled `AgentBehaviorManifest.json` SHA-256 `f050e3b696cc73a82a2c1a9c613efa19ecd16027f0419b81c81ffcafd45817c8`, MSAL `1.9.0`, and signed entitlements. The exported app used Apple Distribution identity `CF9FD3FF6D932AD1EB2822261611570904AE5641`, team `52T7P32J34`, and `get-task-allow=false`. Xcode selected an automatically managed App Store profile with UUID `f16a0f00-406d-476a-9210-ce27e7df387c`, no provisioned devices, and expiration `2027-08-09`; that archive profile is distinct from the separately installed profile UUID `64b7a123-7fe8-48f4-a4a3-8d1aa8bfb91a`. Export logged a nonfatal `Upload Symbols Failed` warning because the embedded MSAL framework UUID `67EC8882-4D2F-33F6-88ED-8B8CEAED65B3` has no matching dSYM in the archive; Lumen and llama symbol packages are present, but MSAL crash symbolication is not proven.

`LUMEN_NO_UPLOAD=1` was a hard boundary for that run, so no upload, TestFlight submission, processing, or App Store acceptance occurred. The archive evidence is bound to source `5df950aeb58088d2cf5a371a0b373a8ca9377d5e`; a later documentation commit must not be described as the archived source. RAG indexing/search, memory extraction/storage, voice, App Intents, real background scheduling, Microsoft OAuth/mailbox behavior, TestFlight behavior, App Store upload, processing, and acceptance remain unproven.
...
- A no-upload Release archive/export from exact source `5df950aeb58088d2cf5a371a0b373a8ca9377d5e`, build `20260810035524`, completed with both `** ARCHIVE SUCCEEDED **` and `** EXPORT SUCCEEDED **`. The archive is `build/Lumen-20260809-235633.xcarchive`; the IPA is `build/export-Lumen-20260809-235633/Lumen.ipa` with SHA-256 `760c1a4f8f7444211229bf73d7804fb1e5dd90ff36082c7f32615b35b27c9a83`. Archive/IPA provenance, Info.plist values, app privacy manifest, bundled manifest SHA, MSAL `1.9.0`, and signed entitlements passed. `LUMEN_NO_UPLOAD=1` prevented upload, so TestFlight, processing, upload, and acceptance remain unproven.

...
## 2026-08-09/10 production-readiness hardening checkpoint
...
This proves only that exact DEBUG diagnostic flow for that source, build, device, and scenario; it is not evidence that the submitted Release surface exposes the flow or that another tool behaves the same way.

Separately, a fresh Release archive/export first completed under a no-upload guard from exact source `635c7b0c2f7f2ee6a697b2622d025153924a7c35` with build number `20260810044413`. `xcodebuild` reported `** ARCHIVE SUCCEEDED **` for `build/Lumen-20260810-004452.xcarchive` and `** EXPORT SUCCEEDED **` for `build/export-Lumen-20260810-004452/Lumen.ipa`; the exported IPA had SHA-256 `0f775f81a8c730ba2848bd9a697ac59b2ac9d1e7493be4b4a132bb77ab956bc1`. The wrapper verified archive and IPA Info.plist/provenance, the app privacy manifest, bundled `AgentBehaviorManifest.json` SHA-256 `f050e3b696cc73a82a2c1a9c613efa19ecd16027f0419b81c81ffcafd45817c8`, MSAL `1.9.0`, and signed entitlements. The exported app used Apple Distribution identity `CF9FD3FF6D932AD1EB2822261611570904AE5641`, team `52T7P32J34`, and `get-task-allow=false`. Xcode selected an automatically managed App Store profile with UUID `f16a0f00-406d-476a-9210-ce27e7df387c`, no provisioned devices, and expiration `2027-08-09`; that export profile is distinct from the separately installed profile UUID `64b7a123-7fe8-48f4-a4a3-8d1aa8bfb91a`.

The release lane installed the official Microsoft MSAL `1.9.0` dSYM only after its release ZIP matched the pinned SHA-256 `ecbb4f3c1e8f7e943cd7bf304b2cbe053bfc9998d41848480d6438218cfb6e12`. Fail-closed UUID coverage passed 3/3 for Lumen, MSAL, and llama in both the archive and exported IPA. `Packaging.log` contains no `Upload Symbols Failed`, so this candidate supersedes the previous archive's MSAL-symbol gap.

`LUMEN_NO_UPLOAD=1` prohibited upload during that archive/export run; the exact IPA was subsequently uploaded under explicit authorization. `altool` completed without errors and returned Delivery UUID/build ID `d75bbaf0-c722-46be-a1df-d29f0bbe47a7`. App Store Connect records upload time `2026-08-09T22:08:12-07:00`, terminal API state `VALID`, internal state `IN_BETA_TESTING`, external state `READY_FOR_BETA_SUBMISSION`, and not expired. The `1.0.1` App Store version record remains `PREPARE_FOR_SUBMISSION` with no selected build; the uploaded binary's marketing version is `1.0.0`, so it cannot be selected for that version record. No external beta submission, App Store review submission, metadata change, or acceptance occurred, and the exact TestFlight build has not been installed or run on a physical iPhone. The archive and upload evidence remain bound to source `635c7b0c2f7f2ee6a697b2622d025153924a7c35`; a later documentation commit must not be described as the archived source. RAG indexing/search, memory extraction/storage, voice, App Intents, real background scheduling, Microsoft OAuth/mailbox behavior, exact-build physical TestFlight behavior, external beta review, and App Store review/acceptance remain unproven.
...
All controlled Unsloth base-model loads now set `use_exact_model_name=True` for SFT, DPO/ORPO, frozen evaluation, and GGUF export. The downstream adapter checks remain strict. Regression coverage requires the literal fail-closed flag at every controlled load site and explicitly rejects the observed Unsloth mirror as an SFT parent for the canonical Qwen configuration.

A GPU-backed probe in the pinned CUDA 12.8/Python 3.10 image loaded the two official Qwen checkpoint shards at the configured revision and produced PEFT and tokenizer identities of `Qwen/Qwen3-1.7B`. The focused lineage/export/evaluation tests, complete Unsloth test directory, non-slow/non-E2E crawler suite, Release-hardening guard, and integration gate passed. The prescribed `uv` runner was unavailable on this host, so the crawler suite ran with `python3 -m pytest` instead. The failed adapter remains mirror-bound and the patch changes the frozen phase-code digests, so that run must not be resumed or re-attested.

The subsequent fresh run, `uidfix2-20260715T065617Z`, proved the exact-model fix through finalized SFT and preference artifacts for Cortex, Executor, Mouth, Mimicry, and REM. It also exposed an independent launcher defect before the expensive frozen evaluation completed: each phase loop consumed `AGENTS_CSV` through an unterminated `printf` stream, so Bash skipped the final `fleet` record even though the run manifest and eventual summary required all six agents. The doomed run was stopped after 19 of 558 Cortex evaluation cases. The launcher now parses the validated CSV into a Bash array once and uses the same complete array for training/preference, frozen evaluation, and GGUF conversion. Regression coverage requires all three loops to use that array and rejects the unterminated-stream idiom. That incomplete run is not end-to-end training, preference, evaluation, or GGUF evidence; only a distinct fresh six-agent run can provide that claim.

The next non-resumed run, `uidfix2-20260715T072059Z`, finalized exact-model SFT and preference artifacts for all six agents, including Fleet, and then exposed a separate frozen-evaluation contract defect. Cortex emitted a malformed JSON object on the second case: its `reasoningSummary` absorbed a sibling `requiresApproval` fragment and the object then repeated that field. A focused two-case replay preserved the raw malformed completion and strict `invalid_json` result under `.local/ubuntu_finetune_runs/diagnostics/uidfix2-20260715T072059Z/cortex-first2`; the evaluator did not extract, rewrite, or accept it. The run was stopped once multiple format failures made the zero-format-failure quality gate impossible, and its lineage must not be resumed.

The generated Cortex targets were not corrupt: every optimized SFT assistant target was a JSON object. The defect was a cross-layer contract mismatch. Cortex's system prompt did not state its JSON-only output contract, several preference pairs taught prose for the same role, dense `reasoningSummary` text echoed sibling schema concepts, and Cortex-owned runtime-selection evals incorrectly demanded Executor-owned root arguments. Frozen evaluation also used unconstrained greedy Transformers decoding. That decoder comparison does not make the offline Cortex route payload an iOS wire contract: Release iOS continues to request its current constrained `action` or `final` JSON object, and the shipped runtime owns routing, clarification, canonical manifest validation, approval, and action persistence.

The hardened offline qualification contract keeps semantic scoring fail-closed while aligning those layers: Cortex training and chosen preference outputs are strict JSON objects; reasoning summaries use ordinary prose rather than serialized key/value fragments; tool schema/argument construction is Executor-owned; offline route selection is Cortex-owned; and the evaluator permits one deterministic fresh generation only after strict raw JSON or manifest-route protocol validation fails. Curated prompts explicitly distinguish supplied, partial, and missing required arguments; Executor schema cases supply deterministic values and score the entire arguments object exactly. Every decoded attempt remains unmodified, prompt-bound, and hash-bound evidence. Promotion reloads the frozen suite and behavior contract, reconstructs the exact final configuration, independently re-scores every candidate, and requires the stored report to equal that reconstruction. It still rejects fences, prefixes, suffixes, arrays, malformed objects, retry exhaustion, evidence/path drift, and semantic failures, and it never retries a semantic metric failure. A canonical summary binds prepared-run lineage, SFT and preference evidence, evaluation evidence when requested, and any GGUF files to their recorded hashes. A full quality pass with verified GGUFs is `status=complete`; the same full quality pass with conversion intentionally disabled is `status=complete_without_gguf`, not a smoke run. Smoke and unevaluated runs remain diagnostic states even when their requested GGUF artifacts verify, and resume reuses those artifacts only from `ggufStatus=verified` plus matching per-agent hash and size evidence. This section does not pre-assert full-pipeline success: an end-to-end training-and-conversion claim is valid only when a fresh non-resumed run produces a canonically verified `aio_summary.json` with `status=complete`, complete frozen evaluations for every agent, and all six GGUF hashes. It still would be offline artifact-lineage evidence, not proof that those artifacts are wired into iOS or that device, TestFlight, or live-runtime validation passed.

The fresh non-resumed run `uidfix3-20260715T091507Z-internal_plus_public_optimized` then completed finalized exact-parent SFT and preference artifacts for all six agents, but Cortex's held-out evaluation exposed a deeper corpus defect. The authoritative run was stopped at case 121 after a held-out trigger-list request exhausted both 1,024-token attempts with recursively growing rejected-tool catalogs. A complete diagnostic replay of the preserved Cortex adapter evaluated all 504 frozen cases and failed closed: 56 passed, 448 critical cases failed, the weighted score was `0.111111`, nine first attempts were malformed, six bounded retries recovered syntax, and three cases remained terminal `invalid_json`. The other terminal cases were the explicit `emailDraft` allowed-tool selection and a second held-out trigger-list request; their raw attempts copied forbidden candidates, invented recursively expanding IDs, or emitted an unbounded timestamp list. The stopped six-agent run and its diagnostic replay are failure evidence only and must never be resumed or promoted.

The diagnostic's overlapping metric failures were 294 tool-selection failures, 162 missing persisted actions, 197 missing or unequal required fields, 69 approval-boundary mismatches, two no-tool failures, and one invalid-tool rejection failure. Source-level mapping found that the old corpus taught actions for missing-required-argument requests, sparse preference targets, singular or repeated rejected-tool catalogs, and no canonical fail-closed targets for the empty `chat`/`unknown` routes or `system.root.delete`. The corrected Cortex corpus now provides 62 complete actionable routes, 46 exact missing-argument clarifications, 20 compact selection-only contrasts, two explicit no-tool routes, one exact invalid-tool rejection, canonical chosen preference routes, train/validation controls for bounded JSON retry, and normalized public chat targets. Training remains disjoint from held-out evaluation under the schema 1.1 hash-only contamination contract: exact record or segment fingerprints, 13-token near-overlap shingles, and 4-token short-window containment checks over non-system content. Validation also verifies the report self-hash, training/evaluation corpus hashes and counts, the hash-only public-evaluation fingerprint bundle SHA/count, variant-manifest bindings, and zero contamination matches.

Frozen evaluation retains the shipped/default non-Fleet repetition penalty of `1.1`, while Fleet qualification uses the neutral value `1.0` because its canonical event graphs must copy the exact scenario ID 11–22 times. The agent-bound value is recorded in independently verified generation evidence; no operator override is accepted. This Fleet-specific offline qualification setting is not an iOS or device-validation claim. The evaluator rejects duplicate JSON object keys and treats any Cortex `rejectedToolID` or `rejectedToolIDs` catalog as a retry-eligible contract failure. Every one of the 504 Cortex cases also carries a critical record-aware offline route metric: actionable cases require an exact persisted action bound to the selected manifest tool, clarification cases require the exact missing arguments and a question with no action, contrast cases require the compact five-field selection shape, and no-tool or invalid-tool cases require their exact null-selection state. Those five-field and `actionStep` shapes qualify the adapter; they do not replace the shipped iOS `action`/`final` wire schema. Candidate JSONL evidence is loaded with the same unique-key policy. Replaying the old adapter with repetition penalty `1.1` made two prior runaway cases syntactically bounded but did not repair their semantics, while the second trigger-list case still failed strict JSON after one retry; only a fresh retrain and unchanged held-out expectations can establish behavioral recovery.

Prelaunch local validation passed after these corrections: 341 Unsloth tests passed with two pinned-environment integration tests skipped, including all 91 adapter-evaluation tests; the focused 145-test Cortex/dataset/Fleet quality selection passed; and the prescribed non-slow/non-E2E crawler suite passed all 492 selected tests with 125 deselected. Python compilation, launcher shell syntax, and `git diff --check` also passed. `uv` is unavailable on this Ubuntu host, so the equivalent `python3 -m pytest` collection and execution paths were used and no `uv.lock` byproduct was created. These checks validate source and deterministic corpus contracts; they do not replace the pending fresh model-backed run.

The current follow-up binds the entire Ubuntu orchestration closure to an exact clean Git snapshot and image, including both launchers, the pipeline helper, trainers, evaluator, exporter, uploader, Dockerfile, Docker ignore rules, and imported behavior-affecting helpers. The credential-scoped uploader executes the verified image copy under isolated Python without a checkout or repository-wide `PYTHONPATH` mount. Evaluation qualification and GGUF conversion now have independent states, and smoke or unevaluated adapters require an explicit diagnostic-upload override and publish only below a diagnostic namespace. Retained-container recovery binds the exact execution mode and environment; checkpoint manifests fail closed over the optimizer, scheduler, RNG, trainer, policy-adapter, and conditional FP16 loss-scaler state. These controls address execution and recovery integrity; they do not predict model behavior.

Mouth and Mimicry text qualification now uses finite audited grounded relation frames rather than token-presence alone. The scorer rejects unsupported qualifiers, relation inversion, bilingual truncation, unsafe appends, markup, emoji, mathematical negation, bidi and zero-width controls, combining overlays, homoglyph changes, interrogative polarity, and extra structured fields while retaining 30 explicit valid phrasings. Fleet directory and delegation cases require their exact top-level schemas, ordered manifested slots, exact owner, and the fixed `manifest_responsibility_match` reason. JSON-mode Mimicry and REM evaluations receive the same structured-output instruction as their optimizer-visible records; text-mode Mimicry does not. An independent adversarial audit produced no false pass across 72 semantic mutations and 23 Fleet schema mutations. These are source-level scorer and curriculum claims, not model-backed pass evidence.

The source-level optimizer audit uses the pinned Qwen tokenizer and mirrors Transformers 4.57.6 and TRL 0.24 tokenization, including the EOS token that DPO appends to each chosen and rejected completion. All six roles pass the exact SFT and DPO preflights with no truncation, duplicate rows, train/validation prompt overlap, identical preference pairs, reversed preference pairs, contamination matches, or production-validator failures. The projected SFT/DPO optimizer steps are Cortex `213/48`, Executor `46/9`, Mouth `36/12`, Mimicry `36/14`, REM `21/8`, and Fleet `34/12`; their frozen evaluation suites contain `507`, `63`, `7`, `5`, `6`, and `15` cases respectively. Fleet's exact training-token mixture is 58.11% behavioral-primary, 18.57% public behavioral, and 23.32% supplemental static for SFT, while DPO is 94.40% behavioral-primary and 5.60% supplemental static. Every supplemental SFT family remains below its independent 10% runtime ceiling. Risk-first execution therefore uses `fleet,executor,mouth,rem,mimicry,cortex`, qualifying and exporting each role before starting the next; this changes failure cost, not all-pass outputs.

Pilot 27 remains historical Cortex smoke evidence only: its adapter passed all 36 generated smoke cases, but it did not run the current 507-case Cortex suite or qualify the other five roles. No end-to-end success is asserted for the follow-up source until a fresh, non-resumed six-role run completes all frozen evaluations, reports zero terminal format failures, verifies every requested GGUF, and independently re-verifies its canonical summary and run manifest after process exit.

A bounded evaluation smoke run records the full frozen case count separately from the cases actually generated. The scored report covers only the generated cohort: generated passes are `passedCaseCount`, generated failures are `generatedCaseCount - passedCaseCount`, and `missingOutputCount` counts missing outputs only within that cohort. Ungenerated cases remain visible as `fullCaseCount - generatedCaseCount`; they are not inserted into the scored report. `criticalFailureCount` is therefore the count of critical failures within the generated cohort, not the total generated-failure count. Consequently, smoke completion cannot support a full-suite claim. Even a future independently verified `603/603` aggregate would establish offline six-adapter evaluation and artifact lineage only, not iOS wiring, a signed build, real-device behavior, TestFlight acceptance, or live tool/RAG/memory/voice/AppIntent behavior.
...
This is not a claim that every future product target is complete. The Release product surface now excludes experimental or legacy paths that are not release-safe, and documents those exclusions explicitly. Signed physical-device compile, risk-focused XCTest, install/process liveness, one exact DEBUG physical model/tool flow, a source-bound signed Release archive/export, and a terminally processed App Store Connect upload now have separate evidence. Physical execution of the exact TestFlight build, external beta submission, App Store version selection/review/acceptance, and broader production model/tool/RAG/memory/voice/App Intent/background/OAuth behavior still require separate evidence.
...
| `git diff --check` after Ubuntu exact-model lineage hardening | Passed |
| `python3 -m compileall tools/fine_tuning/unsloth scripts` after Ubuntu exact-model lineage hardening | Passed |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q -p no:cacheprovider tools/fine_tuning/unsloth/tests` after Ubuntu exact-model lineage hardening | Passed |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q -p no:cacheprovider tools/fine_tuning/unsloth/tests/test_ubuntu_pipeline.py tools/fine_tuning/unsloth/tests/test_train_dpo_lineage.py tools/fine_tuning/unsloth/tests/test_evaluate_adapter.py tools/lumen_manifest_crawler/tests/test_unsloth_export_gguf.py` | Passed |
| Pinned-image GPU probe with `use_exact_model_name=True` for `Qwen/Qwen3-1.7B` at revision `70d244cc86ccca08cf5af4e1e306ecf908b1ad5e` | Passed: PEFT model and tokenizer both retained `Qwen/Qwen3-1.7B` |
| `python3 tools/check_release_hardening.py` after Ubuntu exact-model lineage hardening | Passed |
| `bash scripts/check-lumen-integration-gate.sh` after Ubuntu exact-model lineage hardening | Passed; static iOS readiness ran because Xcode is unavailable on this Ubuntu host |
| `uv run --python 3.12 --with-editable ./tools/lumen_manifest_crawler --with pytest --with pydantic --with typer --with rich pytest -m "not slow and not e2e"` | Not run: `uv` is not installed on this host (`uv: command not found`) |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q -p no:cacheprovider tools/lumen_manifest_crawler -m "not slow and not e2e"` | Passed as the available-host fallback |
| `bash -n scripts/ubuntu_train_lumen_adapters_aio.sh scripts/ubuntu_train_lumen_full_pipeline.sh` after final-agent loop hardening | Passed |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q -p no:cacheprovider tools/fine_tuning/unsloth/tests/test_ubuntu_pipeline.py` after final-agent loop hardening | Passed; includes the six-agent array-loop regression |
| Generated JSONL, adapter-runtime invariant, Release-hardening, integration-gate, and `git diff --check` validation after final-agent loop hardening | Passed; static iOS readiness ran because Xcode is unavailable on this Ubuntu host |
| Focused Cortex JSON-contract, eval-role ownership, missing-argument, bounded-retry evidence, Fleet-boundary, and Unsloth pipeline regressions after frozen-evaluation hardening | Passed |
...
- A Release archive/export from exact source `635c7b0c2f7f2ee6a697b2622d025153924a7c35`, build `20260810044413`, completed with both `** ARCHIVE SUCCEEDED **` and `** EXPORT SUCCEEDED **`. The archive is `build/Lumen-20260810-004452.xcarchive`; the IPA is `build/export-Lumen-20260810-004452/Lumen.ipa` with SHA-256 `0f775f81a8c730ba2848bd9a697ac59b2ac9d1e7493be4b4a132bb77ab956bc1`. Archive/IPA provenance, Info.plist values, app privacy manifest, bundled manifest SHA, MSAL `1.9.0`, and signed entitlements passed. The hash-pinned official MSAL dSYM was installed; archive and IPA UUID coverage passed 3/3 for Lumen, MSAL, and llama; and `Packaging.log` contains no `Upload Symbols Failed`. The later authorized upload completed without `altool` errors under Delivery UUID/build ID `d75bbaf0-c722-46be-a1df-d29f0bbe47a7`; App Store Connect reports terminal `VALID`, internal `IN_BETA_TESTING`, external `READY_FOR_BETA_SUBMISSION`, and not expired. The exact TestFlight build has not been installed or run on a physical iPhone, and no external beta submission, App Store review submission, metadata change, or acceptance occurred. The `1.0.1` App Store version remains `PREPARE_FOR_SUBMISSION` with no selected build because the uploaded binary's marketing version is `1.0.0`.
...
- One exact model-backed local-model/tool/final loop now has physical DEBUG evidence for the permission-safe `alarm.authorization_status` status read. Physical execution evidence is still needed for the exact uploaded TestFlight build, broader permission behavior and native tool groups, voice, App Intents, RAG, memory, and role-adapter behavior.
- Live exact-build device/TestFlight evidence is still needed for real background scheduling and Microsoft OAuth/mailbox behavior.
...
- TestFlight or real-device smoke test of the exact uploaded Release build; the DEBUG diagnostic pass is not a substitute.
- Real-device local-model load and interactive generation through a production user surface in the exact uploaded Release build; the current proof is DEBUG-only.
- Real-device role-adapter switching.
- Live tool-call validation for every tool-capable surface enabled in that uploaded Release build beyond the one DEBUG `alarm.authorization_status` proof.
...
- Voice and AppIntent flows for the exact uploaded Release build.
- Real background scheduling and execution for the exact uploaded Release build.
- Microsoft OAuth and mailbox behavior for the exact uploaded Release build.
- Resolve the `1.0.1` App Store version record versus uploaded `1.0.0` binary mismatch, select an eligible build, and separately complete any external beta submission, App Store review submission/metadata work, and acceptance.
